...
Impl()
{
  ...
  void set(key_type key, val_type val, size_type size)
  {
    ...
    if ((space_used_ + size) <= maxmem_) {
        ...
        unsigned unused = 0;
        auto value_pointer = get(key, unused);
        ...
        {
            (dict_[hasher_(key) % dict_.size()]).push_back(trip);
            if (evictor_){
  ...
  {
    bucket_t current_bucket = dict_[hasher_(key) % dict_.size()];
